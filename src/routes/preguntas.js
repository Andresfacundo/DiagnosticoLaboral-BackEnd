...
const { verificarAuth, esAdmin, asociado, permitirRoles } = require('../middlewares/authMiddleware');

//crear pregunta

router.post('/', verificarAuth, esAdmin,  async (req, res) => {
    try {
...
router.get('/categoria/:categoria', verificarAuth, esAdmin, async (req, res) => {
    try {
...
// editar una pregunta
router.put('/:id', verificarAuth, permitirRoles('admin'), async (req, res) => {
    try {
...
router.delete('/:id', verificarAuth,esAdmin, async (req, res) => {
    try {
        const pregunta = await Pregunta.findByPk(req.params.id);
        if (!pregunta) return res.status(404).json({ error: 'Pregunta no encontrada' });
